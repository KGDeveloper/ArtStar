...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        //计算分享视图的总高度
        if (self.platformArray.count%4 == 0) {
        ...
        for (int i=0; i<self.platformArray.count; i++) {
            ...
            [shareBut setTitleColor:[UIColor colorWithHexString:@"666666"] forState:UIControlStateNormal];
...
{
    DXSharePlatform *selfFrind= [[DXSharePlatform alloc] init];
    selfFrind.iconStateNormal = @"weixin_allshare";
    selfFrind.iconStateHighlighted = @"weixin_allshare_night";
    selfFrind.sharePlatform = DXShareTypeWechatSession;
    selfFrind.name = @"APP好友";
    [self.platformArray addObject:selfFrind];
    
    DXSharePlatform *selfTimeLine = [[DXSharePlatform alloc] init];
    selfTimeLine.iconStateNormal = @"weixin_allshare";
    selfTimeLine.iconStateHighlighted = @"weixin_allshare_night";
    selfTimeLine.sharePlatform = DXShareTypeWechatSession;
    selfTimeLine.name = @"APP朋友圈";
    [self.platformArray addObject:selfTimeLine];
    
//#warning 防止审核失败 最好要先判断是否已安装微信、QQ 或其他平台的App
    //微信好友
//    if ([[UMSocialManager defaultManager] isInstall:UMSocialPlatformType_WechatSession]) {
        DXSharePlatform *wechatSessionModel = [[DXSharePlatform alloc] init];
        wechatSessionModel.iconStateNormal = @"share_wechatSession";
...
//    if ([[UMSocialManager defaultManager] isInstall:UMSocialPlatformType_WechatTimeLine]) {
        ...
        wechatTimeLineModel.iconStateNormal = @"share_wechatTimeline";
...
//    if ([[UMSocialManager defaultManager] isInstall:UMSocialPlatformType_QQ]) {
        ...
        qqModel.iconStateNormal = @"share_QQ";
...
//    if ([[UMSocialManager defaultManager] isInstall:UMSocialPlatformType_Qzone]) {
        ...
        qqZone.iconStateNormal = @"share_QQZone";
...
//    }
    //微博
    //    if ([[UMSocialManager defaultManager] isInstall:UMSocialPlatformType_Qzone]) {
    DXSharePlatform *weiBo = [[DXSharePlatform alloc] init];
    weiBo.iconStateNormal = @"share_weibo";
    weiBo.sharePlatform = DXShareTypeQzone;
    weiBo.name = @"微博";
    [self.platformArray addObject:weiBo];
    //    }
    
    //复制链接
//    if (self.shareConentType != DXShareContentTypeImage) {
        DXSharePlatform *urlModel = [[DXSharePlatform alloc] init];
        urlModel.iconStateNormal = @"share_url";
