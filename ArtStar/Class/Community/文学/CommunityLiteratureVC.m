...
@interface CommunityLiteratureVC ()<MusicInstitutionsViewDelegate,UITextFieldDelegate>
...
@implementation CommunityLiteratureVC
...
- (void)setTopView{
    ...
    scrollerView.itemArr = @[@"头条",@"活动",@"文学人"];
    //MARK:--------------------------------------------滚动条右侧按钮点击事件--------------------------------------------
    scrollerView.rightAction = ^(NSString *title) {
        if ([title isEqualToString:@"文学人"]){
...
- (MusicInstitutionsView *)institutionsView{
    ...
    if (!_institutionsView) {
        ...
        _institutionsView.chooseStyle = @"文学";
