...
- (void)setupUI
{
    ...
    [self.cancelBtn setImage:Image(@"false") forState:UIControlStateNormal];
    ...
    [self.doneBtn setImage:Image(@"true") forState:UIControlStateNormal];
...
- (void)viewDidLayoutSubviews
{
    ...
    self.toggleCameraBtn.frame = CGRectMake(kViewWidth-50, NavTopHeight, 30, 30);
    self.flashBtu.frame = CGRectMake(kViewWidth-50, NavTopHeight + 50, 30, 30);
}
...
- (void)setupUI
{
    ...
    self.flashBtu = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.flashBtu setImage:Image(@"闪光灯关闭") forState:UIControlStateNormal];
    [self.flashBtu addTarget:self action:@selector(flashBtuAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.flashBtu];
...
- (void)flashBtuAction:(UIButton *)sender{
    if ([sender.currentImage isEqual:Image(@"闪光灯关闭")]) {
...
- (UIImage *)getScreenShotImageFromVideo{
    UIImage *shotImage;
