...
-(void)setUpUI{
    ...
    self.buttomView = [[UIView alloc]initWithFrame:CGRectMake(0, ViewHeight(self) - 20, kScreenWidth, 20)];
    self.buttomView.backgroundColor = [UIColor clearColor];
    [self addSubview:self.buttomView];
    
    [self updateViewFrameSetting];
}
- (void)setButtomViewColor:(UIColor *)buttomViewColor{
    _buttomViewColor = buttomViewColor;
    self.buttomView.backgroundColor = buttomViewColor;
}
...
-(UIScrollView *)mainScrollView {
    if (!_mainScrollView) {
        _mainScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height - 20)];
